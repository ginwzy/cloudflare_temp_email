...
const { t } = useI18n({
  messages: {
    en: {
      ...
      noAttachments: 'No attachments stored',
    },
    zh: {
      download: '下载',
      action: '操作',
...
<template>
  <div class="attachment-settings">
    <n-modal v-model:show="showDownload" preset="dialog" :title="t('download')">
      ...
      <n-button tag="a" target="_blank" tertiary type="info" size="small" :download="curRow.key?.replace('/', '_')" :href="curDownloadUrl">
        {{ t('download') }}
      </n-button>
    </n-modal>
    <div v-if="data.length === 0" class="empty-shell">
      <n-empty :description="t('noAttachments')" />
    </div>
    <div v-else class="ds-table-shell">
